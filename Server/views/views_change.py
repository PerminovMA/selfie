...
from Server.tools import make_error, token_update
...
def change_avatar_request(request):
    ...
    if photo_file.size > MAX_FILE_SIZE or (preview_file is not None and preview_file.size > MAX_FILE_SIZE):
        return HttpResponse(
    ...
    try:
        ...
        if token == user.token.token:
            if user.avatar.name == '':
                ...
                if preview_file is not None and user.avatar_preview.name == '':
                    user.avatar_preview = preview_file
                elif preview_file is not None and user.avatar_preview.name != '':
                    user.avatar_preview.delete()
                    user.avatar_preview = preview_file
